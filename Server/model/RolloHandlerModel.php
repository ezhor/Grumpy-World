...
class RolloHandlerModel
{
    public static function getRollo($id){
        ...
        $query = "SELECT U.Usuario AS Nombre, S.Nombre AS Sombrero, A.Nombre AS Arma, Z.Nombre AS Zona, R.Nivel, rangoRollo(R.Honor, R.ID_Usuario) AS Rango
                    FROM Usuarios AS U
        ...
        $prep_query->bind_result($nombre, $sombrero, $arma, $zona, $nivel, $rango);
        $prep_query->execute();
        $prep_query->fetch();
