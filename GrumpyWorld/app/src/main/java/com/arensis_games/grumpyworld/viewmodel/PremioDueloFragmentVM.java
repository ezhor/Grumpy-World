...
import com.arensis_games.grumpyworld.connection.PremioInterface;
...
public class PremioDueloFragmentVM extends AndroidViewModel {
    private MutableLiveData<Integer> ldPremio;
    private MutableLiveData<String> ldError;

    public PremioDueloFragmentVM(@NonNull Application application) {
        super(application);
        this.ldPremio = new MutableLiveData<>();
        this.ldError = new MutableLiveData<>();
    }

    public MutableLiveData<Integer> getLdPremio() {
        return ldPremio;
    }
    ...
    public void obtenerPremio(int id){
        OkHttpClient client;
        Retrofit retrofit;
        ...
        */
        if(GestoraToken.getAuthorization() != null){
            ...
            dueloInterface.getPremio(id).enqueue(new Callback<Integer>() {
                @Override
